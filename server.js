...
fastify.get("/api/books", async (request, reply) => {
  ...
  if (!authToken) {
    return reply.status(400).send({
      ...
      message:
        "Please provide a token parameter or set HARDCOVER_TOKEN environment variable",
    });
  }
...
const GET_LIST_BOOKS_QUERY = gql`
  query GetListBooks(
    ...
    list_books(
      where: { list: { name: { _eq: $listName }, user_id: { _eq: $userID } } }
      offset: $offset
      limit: $limit
...
fastify.get("/api/owned", async (request, reply) => {
  ...
  try {
    ...
    while (true) {
      ...
      if (
        !data.list_books ||
        !Array.isArray(data.list_books) ||
        data.list_books.length === 0
      ) {
        return reply.status(404).send({
...
fastify.get("/api/tbrbooks", async (request, reply) => {
  ...
  try {
    ...
    const tbrLists = listsData.lists.filter((list) =>
      list.name.startsWith("TBR:")
    );
    ...
    for (const tbrList of tbrLists) {
      ...
      while (true) {
        ...
        const listBooksData = await graphqlRequest(
          HARDCOVER_API_URL,
          GET_LIST_BOOKS_QUERY,
          variables,
          {
        ...
        if (
          listBooksData.list_books &&
          Array.isArray(listBooksData.list_books)
        ) {
        ...
        if (
          !listBooksData.list_books ||
          listBooksData.list_books.length < limit
        ) {
          break;
        }
...
fastify.get("/api/seriesstatus", async (request, reply) => {
  ...
  try {
    ...
    const seriesLists = listsData.lists.filter((list) =>
      list.name.toLowerCase().startsWith("series: ")
    );
...
fastify.post("/api/series", async (request, reply) => {
  ...
  if (!authToken) {
    return reply.status(400).send({
      error: "User token is required",
      message:
        "Please provide a token in the request body or set HARDCOVER_TOKEN environment variable",
    });
  }
  ...
  const invalidIds = seriesIds.filter(
